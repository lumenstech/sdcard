...
# ---- Network check -------------------------------------------
is_online() {
    # Primary: spider the backend health endpoint. Confirms DNS + TCP.
    if wget -q --spider --timeout=5 "${BACKEND_URL}/health" 2>/dev/null; then
        return 0
    fi
    # Fallback 1: nslookup if available on this busybox build.
    if command -v nslookup >/dev/null 2>&1; then
        if nslookup ingest.secure4k.com >/dev/null 2>&1; then
            return 0
        fi
    fi
    # Fallback 2: ping the default gateway (works even without DNS).
...
# ---- Process buffer ------------------------------------------
process_buffer() {
    # Use a temp file to enumerate so we get a reliable count regardless of
    # busybox quirks in find/echo/wc. POSIX-safe.
    LIST_FILE="${BUFFER_DIR}/.upload_list"
    find "${BUFFER_DIR}" -maxdepth 1 -name "frame_*.jpg" 2>/dev/null | sort > "${LIST_FILE}"
